...
{
    mAUX_Reg[static_cast<uint32_t>(hal::AUX_Reg::ENABLES)] |= (1 << static_cast<uint32_t>(aux_peripheral));
}
...
{
    mAUX_Reg[static_cast<uint32_t>(reg_idx)] = value;
}

uint32_t CAUX::Get_Register(hal::AUX_Reg reg_idx)
{
    return mAUX_Reg[static_cast<uint32_t>(reg_idx)];
}
